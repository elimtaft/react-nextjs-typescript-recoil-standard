...
require 'RESOLVER'
config_subst 'RESOLVER'
require 'NEXTJS_HOST'
config_subst 'NEXTJS_HOST'
require 'ENVIRONMENT'
config_subst 'ENVIRONMENT'
require 'STRAPI_HOST'
config_subst 'STRAPI_HOST'
require 'NGINX_STRAPI_SERVER_NAME'
config_subst 'NGINX_STRAPI_SERVER_NAME'
...
eval sed ${SED_ARGS} \
    < /etc/nginx/conf.d/nginx-strapi.conf.template \
    > /etc/nginx/conf.d/nginx-strapi.conf
...
eval sed ${SED_ARGS} \
    < /etc/nginx/snippets/general-security-header.conf.template \
    > /etc/nginx/snippets/general-security-header.conf

eval sed ${SED_ARGS} \
    < /robots-${ENVIRONMENT}.txt.template \
